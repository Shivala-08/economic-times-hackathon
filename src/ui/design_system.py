"""Shared Design System — Premium UI Components for Industrial Knowledge Intelligence.

Eliminates CSS/HTML duplication across Streamlit pages. Import and use:

    from src.ui.design_system import (
        inject_global_css, hero_header, gradient_divider,
        section_header, confidence_badge, fmt_time, post_feedback,
        sidebar_brand, hackathon_badge, entity_card, stat_chip,
        neighbor_row, path_result, breadcrumb_trail, doc_card,
        citation_card, keypoints_box, llm_status_pill,
        particle_starfield,
    )
"""
...
# ══════════════════════════════════════════════════════════════════════════════
# CSS — Single source of truth for the entire design system
# ══════════════════════════════════════════════════════════════════════════════
...
def post_feedback(question: str, answer: str, rating: int, api_url: str = "http://localhost:8000"):
    try:
        requests.post(f"{api_url}/feedback", json={"question": question, "answer": answer, "rating": rating}, timeout=5)
    except Exception:
        pass
...
# ══════════════════════════════════════════════════════════════════════════════
# PARTICLE STARFIELD — Inline Canvas 2D inside the hero
# ══════════════════════════════════════════════════════════════════════════════
...
def hero_header(
    ...
    right_section = extra_right or (
        ...
        '<div style="font-size:1.3rem;font-weight:800;color:#818cf8;">🏭</div>'
        '<div style="font-size:0.68rem;color:#64748b;text-transform:uppercase;letter-spacing:0.06em;">Platform</div></div>'
    )
    ...
    <div style="position:relative;z-index:2;display:flex;justify-content:space-between;align-items:center;">
        <div>
            <div style="display:flex;align-items:center;gap:0.8rem;margin-bottom:0.5rem;">
                <span class="hackathon-badge">{badge_text}</span>
            </div>
...
<div class="sidebar-brand">
    ...
    <div style="margin-top:0.4rem;"><span class="hackathon-badge">{badge}</span></div>
</div>
""", unsafe_allow_html=True)
...
def sidebar_footer(text: str = "© 2026 Industrial Knowledge Engine", subtext: str = "Built with ❤ for ET Hackathon"):
    import streamlit as st
    st.markdown(f'<div style="text-align:center;padding:0.8rem;font-size:0.68rem;color:#475569;">{text}<br><span style="color:#6366f1;">{subtext}</span></div>', unsafe_allow_html=True)
...
def llm_status_pill(model: str, available: bool, mode: str = "ollama"):
    ...
    if available:
        ...
        st.markdown(f'<div class="llm-pill-on">🟢 {label} &nbsp;|&nbsp; <b>{model}</b></div>', unsafe_allow_html=True)
    else:
